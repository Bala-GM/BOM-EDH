...
def pro_1():
    ...
    def extract_shapescid(description):
        ...
        desired_shapes = {"Transformer", "SOD", "SOT", "CAP", "COG", "NPO", "MLCC", "X7R", "IND" ,"Ferrite", "Choke", "Thermistor",
                          "Diode", "Rectifier", "Zener", "MELF", "Fuse", "varistor", "Transistor", "MOSFET", "RES"} #"""DIP,SDIP"""
    ...
    def extract_shapesot(description): #Transistor
        ...
        desired_shapes = {"SOT-23", "TO-236AB", "SOT-23F", "SOT-323", "SC-70", "SOT-523", "SC-89", "SOT-523W", "SC-75", "SOT-883L", "X2-DFN1006-3", 
                         ...
                         "PowerPAK", "TO-220-3", "TO-220AB", "SOT23-3"}
        for shape in desired_shapes:
            if shape.lower() in str(description).lower():
                return shape
        return None
    ...
    def extract_shapeic(description): #IC
        ...
        desired_shapes = {"SOIC", "UQFN", "LQFP", "SOP", "TSOP", "QFP", "QFN", "BGA", "µBGA", "CBGA", "PBGA", "CSP", "DFN", "PGA", "LGA", "SiP", "MCM", "DIP",
                          ...
                          "SOIC8", "8SOIC", "SOIC-8", "8-SOIC",
                          "SOIC12", "12SOIC", "SOIC-12", "12-SOIC",
                          "SOIC14", "14SOIC", "SOIC-14", "14-SOIC",
                          "SOIC16", "16SOIC", "SOIC-16", "16-SOIC",}
        for shape in desired_shapes:
            if shape.lower() in str(description).lower():
                return shape
        return None
    ...
    def process_excel():
        ...
        replactypeCID = {"SOD": "DIODE", "SOT": "TRANSISTOR", "MOSFET": "TRANSISTOR", "NPO": "CAPACITOR", "COG": "CAPACITOR", "MLCC": "CAPACITOR",
                         "Ferrite": "INDUCTOR", "Choke": "INDUCTOR", "Rectifier": "DIODE", "X7R": "CAPACITOR", "MELF": "DIODE",
        ...
        # Reorder columns
        df = df[['CRD', 'DES', 'MFG', 'MPN', 'QTY', 'TYPE', 'CIDC', 'SHP', 'SHP-C', 'SHP-D', 'SHP-T', 'SHP-I', 'SHP-PIN', 'SHP-POS']]
        ...
        sdj_valuesdid5 = {"SOT-953", "SOT-89-5", "SOT-23-5", "SC-88A"}
        ...
        sdj_valuesdid6 = {"SOT-26", "SC-74", "SOT-363", "SSC-88", "SOT-563", "SC-89-6", "SOT-963", "TLM621"}
        ...
        # Reorder columns
        df = df[['CRD', 'DES', 'MFG', 'MPN', 'QTY', 'TYPE', 'CID', 'PKG', 'SDJ-A', 'SDJ-M']]
        
        df["SDJ"] = df[['SDJ-A', 'SDJ-M']].apply(lambda x: '-'.join(x.dropna().astype(str)), axis=1)
        
        df['SDJ'] = df['SDJ'].str.replace('16-Pin-16-Pin', '16-Pin', regex=False)
        df['SDJ'] = df['SDJ'].str.replace('14-Pin-14-Pin', '14-Pin', regex=False)
        df['SDJ'] = df['SDJ'].str.replace('12-Pin-12-Pin', '12-Pin', regex=False)
        df['SDJ'] = df['SDJ'].str.replace('10-Pin-10-Pin', '10-Pin', regex=False)
        df['SDJ'] = df['SDJ'].str.replace('8-Pin-8-Pin', '8-Pin', regex=False)
        df['SDJ'] = df['SDJ'].str.replace('6-Pin-6-Pin', '6-Pin', regex=False)
        df['SDJ'] = df['SDJ'].str.replace('5-Pin-5-Pin', '5-Pin', regex=False)
        df['SDJ'] = df['SDJ'].str.replace('4-Pin-4-Pin', '4-Pin', regex=False)
        df['SDJ'] = df['SDJ'].str.replace('3-Pin-3-Pin', '3-Pin', regex=False) 
        df['SDJ'] = df['SDJ'].str.replace('2-Pin-2-Pin', '2-Pin', regex=False)
        df['SDJ'] = df['SDJ'].str.replace('-2-Pin', '2-Pin', regex=False)
        df['SDJ'] = df['SDJ'].str.replace('POS-', 'POS', regex=False)
        
        #df['M_R1'] = df.apply(lambda row: 'OK' if row['SDJ-A'] == row['SDJ-M'] else '', axis=1)
        #df['M_R2'] = df.apply(lambda row: 'OK' if row['SDJ-A'] == row['SDJ-M'] else 'Mismatch', axis=1)

        # Reorder columns
        df = df[['CRD', 'DES', 'MFG', 'MPN', 'QTY', 'TYPE', 'CID', 'PKG', 'SDJ-A', 'SDJ-M', 'SDJ']]
